...
usage() {
    echo ". set-env.sh   ORG_NAME   [PEER_NAME default=5bloques-peer1]  [IDENTITY default=admin] [PORT_NUMBER_BASE default=7050] [ORDERER_ADDRESS default=orderer.5bloques.com:7050] "
...
if [ -z $2 ];
then
    echo "Switching PEER_NAME for Org = 5bloques-peer1"
    PEER_NAME=5bloques-peer1
else
    PEER_NAME=$2
fi
...
if [ -z $5 ]
then
    echo "Setting ORDERER_ADDRESS=orderer.5bloques.com:7050"   
...
# Capitalize the first letter of Org name e.g., 5bloques => 5bloques  seradpa => seradpa
